...
- دمج مسار Startup/elevation داخل `Switch.exe` بدلاً من تثبيت برنامج مساعد منفصل.
- إصلاح تسجيل Scheduled Task باستخدام XML صالح، مع تشغيل التطبيق عند تسجيل الدخول وإعادة تشغيله تلقائياً عند الإغلاق غير الطبيعي.
- تحصين Keyboard Hook وواجهة المستخدم والمهام الخلفية من الاستثناءات التي كانت قد تؤدي إلى إغلاق البرنامج.
- تنظيف تسجيلات startup القديمة وملف `SwitchStartup.exe` أثناء التثبيت والترقية.
...
- ينشئ المثبّت Scheduled Task بصلاحية المسؤول ليبدأ `Switch.exe` تلقائيًا في الخلفية عند تسجيل الدخول إلى ويندوز، ويضيف اختصارًا ظاهرًا في مجلد Startup كي يظهر Switch في قائمة Startup Apps. الاختصار يستدعي `Switch.exe --startup`، ويتولى الملف نفسه تشغيل المهمة المرتفعة أو استخدام تشغيل احتياطي بصلاحية المسؤول. كما تُضبط المهمة بلا حد زمني مع إعادة تشغيلها عند الإغلاق غير الطبيعي. يمكن مراجعة المهمة من Task Scheduler باسم `Switch`، أو داخل مجلد `Microsoft\Windows\Switch` على الأجهزة التي لا تتيح مجلد المهام الجذر.
